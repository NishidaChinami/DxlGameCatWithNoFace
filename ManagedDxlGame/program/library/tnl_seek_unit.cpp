...
namespace tnl {
	...
	void SeekUnit::restart() {
		...
		uint32_t start_frame = (ePlayDir::REVERSE == play_dir_) ? total_frame_num__ - 1 : 0;
	...
	void SeekUnit::jumpSeekFrame(uint32_t frame) {
		frame = std::clamp<uint32_t>(frame, 0, (total_frame_num__ - 1));
		seek_position_ = (static_cast<double>(frame) / static_cast<double>(total_frame_num__)) * total_time_;
		seek_position_ = std::clamp<double>(seek_position_, 0, total_time_);
		if (frame_functions_storage_.empty()) return;
	...
	void SeekUnit::Update(const double delta_time) {
		...
		Update_frame %= total_frame_num__;
	...
	void SeekUnit::seekProcess(const float delta_time) {
		...
		if (0 >= time || time >= total_time_) {
			...
			if (ePlayMode::SINGLE == play_mode_) {
				seek_position_ = time;
				if (0 == total_frame_num__) {
					seek_position_ = std::clamp(seek_position_, 0.0, total_time_);
				}
				else {
					seek_position_ = std::clamp(seek_position_, 0.0, total_time_ - (total_time_ / total_frame_num__));
				}
				stop();
			}
				...
				direction_ *= -1;
				seek_position_ += ((total_time_ / total_frame_num__) * static_cast<double>(direction_)) - over_time;
			}
		}
		else {
			seek_position_ = time;
	...
	//-------------------------------------------------------------------------------------------------------
	void SeekUnit::setTotalFrameNum(const uint32_t frame_num_) {
		if (0 == frame_num_) {
			total_frame_num__ = static_cast<uint32_t>(total_time_ / (1.0 / fix_fps_));
			return;
		}
		total_frame_num__ = frame_num_;
		frame_functions_storage_.resize(total_frame_num__);
	}

	//-------------------------------------------------------------------------------------------------------
	double SeekUnit::getSeekRateInFrame() {
		double frame_time = total_time_ / static_cast<double>(total_frame_num__);
	...
	uint32_t SeekUnit::getSeekFrame(const eFrameType& e_frame) {
		...
		uint32_t frame_now = static_cast<uint32_t>(std::floor(static_cast<double>(total_frame_num__) * getSeekRate()));
		if (eFrameType::CURRENT == e_frame) return frame_now % total_frame_num__;

		int32_t dir = (eFrameType::NEXT == e_frame) ? direction_ : -direction_;

		uint32_t frame = static_cast<int>(frame_now) + dir;
		if (total_frame_num__ <= frame || 0 > frame) {
			if (ePlayMode::SINGLE == play_mode_) {
				frame = std::clamp<int32_t>(frame, 0, total_frame_num__ - 1);
			}
			else if (ePlayMode::REPEAT == play_mode_) {
				frame %= static_cast<int>(total_frame_num__);
				frame = static_cast<uint32_t>(std::abs(static_cast<int>(frame))) * (total_frame_num__ - 1);
			}
			else if (ePlayMode::REFLECTION == play_mode_) {
				frame += -(dir * 2);
			}
